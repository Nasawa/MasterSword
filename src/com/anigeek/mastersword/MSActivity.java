...
{	
    ...
    public void onCreate(Bundle savedInstanceState)// Basically main
	{
        ...
        setContentView(R.layout.msword);// gets the .xml used to make the layout
        zelda = (ImageView)findViewById(R.id.mSword);// dunno why I called it Zelda. It's basically the picture of the sword
        back = "MSA";// makes it so I can return from preferences
        startup();// calls startup
       
        blarg = false;// necessary for the swords. Helps with charge.
//        if(!blarg)// just a precaution
//        	charge.stop();// clears charge
//        else// if blarg
//        	blarg = false; // make blarg false
        
        AdView adView = (AdView)this.findViewById(R.id.ad);// finds the AdView
        ...
        try
        {
	        ...
	        voiceNum = 6;// number of sounds which are voices. Voices MUST be at the top.
	        
	        attack = init;// sets it to be static so I can use it throughout the program.
	        
	        mShaker = new ShakeActivity(this);// Shake Activity
	        mShaker.setOnShakeListener(new ShakeActivity.OnShakeListener()// calls if... shaken? Shook? Whatever. 
	        {
	          public void onShake()// when shook-shaken
	          {
					if(lefty)// if... lefty
					{
					...
					else
					{
		      		...
		      		shake();// call the shake method from BaseActivity
	          }
	        });
